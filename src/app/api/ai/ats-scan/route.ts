...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error('ATS scan error:', isTimeout ? 'Request timed out' : err)
    await refundCredits(userId, CREDIT_COSTS.AI_ATS_SCAN, 'AI_ATS_SCAN', isTimeout ? 'AI request timed out' : 'ATS scan failed')
    return NextResponse.json({ error: isTimeout ? 'AI request timed out. Please try again.' : 'ATS scan failed' }, { status: isTimeout ? 504 : 500 })
  }
}
