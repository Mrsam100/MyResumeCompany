...
import type { TemplateColors, TemplateFonts } from '@/types'
...
interface SingleColumnOptions {
  ...
  headerExtra?: (info: TemplateProps['content']['personalInfo'], colors: TemplateColors, fonts: TemplateFonts) => React.ReactNode
  summaryStyle?: (colors: TemplateColors) => React.CSSProperties
  sectionTitleStyle?: (colors: TemplateColors, fonts: TemplateFonts) => React.CSSProperties
  dividerStyle?: (colors: TemplateColors) => React.CSSProperties
  topAccent?: (colors: TemplateColors) => React.ReactNode
}

export function createSingleColumnTemplate(options: SingleColumnOptions = {}) {
  return function SingleColumnTemplate({ content, config, customColors, customFonts }: TemplateProps) {
    ...
    return (
      <div style={{ width: '100%', minHeight: '100%', backgroundColor: colors.background, fontFamily: fonts.body, color: colors.text, padding: `${spacing.margins}px`, fontSize: '12px', lineHeight: 1.5 }}>
        {options.topAccent?.(colors)}

        <TemplateHeader info={content.personalInfo} colors={colors} fonts={fonts} layout={options.headerLayout ?? 'left'} />
        {options.headerExtra?.(content.personalInfo, colors, fonts)}

        {content.personalInfo.summary && (
          <div style={{ marginTop: `${spacing.sectionGap}px`, ...(options.summaryStyle?.(colors) ?? {}) }}>
            {!options.summaryStyle && (
              <div style={{ ...(options.dividerStyle?.(colors) ?? { borderBottom: `1.5px solid ${colors.primary}`, paddingBottom: '3px', marginBottom: '6px' }) }}>
