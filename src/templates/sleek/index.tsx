...
export function Sleek({ content, config, customColors, customFonts }: TemplateProps) {
  const colors = { ...config.defaultColors, ...customColors }
  const fonts = { ...config.defaultFonts, ...customFonts }
  const spacing = config.spacing
  const { personalInfo } = content
  const sidebarText = colors.sidebarText ?? '#f1f5f9'
  ...
  const sidebar = (
    <div style={{ fontFamily: fonts.body, fontSize: '11px' }}>
      ...
      {personalInfo.title && <p style={{ fontSize: '11px', color: colors.secondary, marginTop: '4px' }}>{personalInfo.title}</p>}
