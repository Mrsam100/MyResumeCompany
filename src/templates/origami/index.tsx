...
export function Origami({ content, config, customColors, customFonts }: TemplateProps) {
  ...
  const sidebarSections = content.sections.filter((s) => s.visible && s.entries.length > 0 && ['skills', 'languages', 'interests'].includes(s.type))
  const mainSections = content.sections.filter((s) => s.visible && s.entries.length > 0 && !['skills', 'languages', 'interests'].includes(s.type))

  const sidebar = (
    <div style={{ fontFamily: fonts.body, fontSize: '11px' }}>
      {personalInfo.fullName && <h1 style={{ fontSize: '17px', fontWeight: 700, color: sidebarText, fontFamily: fonts.heading }}>{personalInfo.fullName}</h1>}
      {personalInfo.title && <p style={{ fontSize: '10px', color: colors.secondary, marginTop: '3px' }}>{personalInfo.title}</p>}
      {contactItems.length > 0 && (
        <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {contactItems.map(({ v, I: Icon }, i) => (
            <span key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px', color: sidebarMuted, fontSize: '9px' }}><Icon size={9} />{v}</span>
          ))}
        </div>
      )}
      {sidebarSections.map((section) => (
        <div key={section.id} style={{ marginTop: '16px' }}>
          <h2 style={{ fontSize: '9px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: colors.secondary, marginBottom: '6px' }}>{section.title}</h2>
  ...
  return <SidebarLayout colors={colors} fonts={fonts} spacing={spacing} sidebar={sidebar} main={main} side="right" />
}
