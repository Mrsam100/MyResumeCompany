...
export function RefinedGold({ content, config, customColors, customFonts }: TemplateProps) {
  ...
  const sidebar = (
    <div style={{ fontFamily: fonts.body, fontSize: '11px' }}>
      ...
      {personalInfo.title && <p style={{ fontSize: '11px', color: colors.secondary, marginTop: '2px', fontStyle: 'italic' }}>{personalInfo.title}</p>}
      <div style={{ width: '30px', height: '2px', background: colors.secondary, marginTop: '10px' }} />
      ...
      {sidebarSections.map((section) => (
        <div key={section.id} style={{ marginTop: '18px' }}>
          <h2 style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: colors.secondary, marginBottom: '6px' }}>{section.title}</h2>
  ...
  const main = (
    <div style={{ fontFamily: fonts.body, fontSize: '12px' }}>
      {personalInfo.summary && (
        <p style={{ fontSize: '11px', color: colors.textLight, lineHeight: 1.6, marginBottom: `${spacing.sectionGap}px`, borderLeft: `3px solid ${colors.secondary}`, paddingLeft: '12px' }}>{personalInfo.summary}</p>
      )}
