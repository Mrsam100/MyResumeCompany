...
const checks: { name: string; check: () => boolean; required: boolean }[] = [
  ...
  { name: 'RAZORPAY_KEY_ID set', check: () => !!process.env.RAZORPAY_KEY_ID, required: true },
  { name: 'RAZORPAY_KEY_SECRET set', check: () => !!process.env.RAZORPAY_KEY_SECRET, required: true },
  { name: 'RAZORPAY_WEBHOOK_SECRET set', check: () => !!process.env.RAZORPAY_WEBHOOK_SECRET, required: true },
  { name: 'NEXT_PUBLIC_RAZORPAY_KEY_ID set', check: () => !!process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID, required: true },
